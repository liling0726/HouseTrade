...
import org.tarena.service.impl.HouseServiceImpl;

public class SearchPage extends SimpleTagSupport{
	...
	public void doTag() throws JspException, IOException {
		...
		if(params == null || "".equals(params.trim())){
			...
			for(int i=0;i<1;i++){
				...
				out.println("<dd>");
				out.println("<a class='cur' href='"+contextPath+"/'><em>1</em></a>");
			...
			if(flag){//参数路径包含数字
				for(int i=0;i<1;i++){
					...
					if(containsSC == null){//不在同一组
						...
						List<SearchCondition> scList = scg.getSearchConditions();
						out.println("<dd>");
						out.println("<a class='cur' href='"+contextPath+"/"+params+"/'><em>1</em></a>");
						for(int j=0;j<PAGE_NUM;j++){
							SearchCondition sc = scList.get(j);
						...
						if(params.length() == containsSC.length()){
							out.println("<a href='"+contextPath+"/'><em>1</em></a>");
						}else{
							out.println("<a href='"+contextPath+"/"+params.replace(containsSC,"")+"/'><em>1</em></a>");
						}
						for(int j=0;j<PAGE_NUM;j++){
							SearchCondition sc = scList.get(j);
							if(params.contains(sc.getScid())){
